...
object AnrealCopy {
    ...
    fun get(key: String): String {
        return when (key) {
            ...
            LOGIN_SUBTITLE -> "Sign in with the email and password for your Anreal workspace."
            REGISTER_TITLE -> "Create your workspace"
            REGISTER_SUBTITLE -> "A name, email, and password. Then you can ask about your documents."
            ...
            CD_APP_MARK -> "Anreal"
            BOARDING_DOCUMENTS_TITLE -> "Ask anything about your documents"
            BOARDING_DOCUMENTS_BODY ->
                "Upload a PDF or image. Answers stay grounded, with citations you can open."
            BOARDING_WEB_TITLE -> "Search the web in the same thread"
            BOARDING_WEB_BODY ->
                "Turn on web search when a file is not enough. Sources stay visible as tool steps."
            BOARDING_IMAGES_TITLE -> "Generate images while you chat"
            BOARDING_IMAGES_BODY ->
                "Create or edit a visual in the thread, then keep talking about it."
            CD_BOARDING_PAGE -> "Feature {0} of {1}"
            BOARDING_USER_PROMPT -> "Where is the revenue risk in the board pack?"
            BOARDING_ASSISTANT_REPLY ->
                "Risk concentrates in enterprise renewals — two accounts delayed."
            BOARDING_CITATION -> "pp. 12–14"
            BOARDING_WEB_TOOL -> "Searching the web"
            BOARDING_WEB_SOURCE -> "4 sources · last 30 days"
            BOARDING_IMAGE_CAPTION -> "Risk heatmap"
            ACTION_RETRY -> "Retry"
